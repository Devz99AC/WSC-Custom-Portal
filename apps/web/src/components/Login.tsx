...
export function Login() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setStatus("sending");
    try {
      // "sent" → link on its way; "denied" → the email has no portal access (no active,
      // non-cancelled order). The BFF tells us which; we show a different card for each.
      setStatus(await requestMagicLink(trimmed));
    } catch {
      setStatus("error");
    }
  }
  ...
  if (status === "denied") {
    return (
      <div className="login">
        <div className="login-card">
          <WscLogo variant="full" />
          <div className="login-tick login-cross" aria-hidden="true">
            ✕
          </div>
          <h1 className="disp">We couldn&apos;t sign you in</h1>
          <p className="sub">
            <strong className="login-echo">{email}</strong> isn&apos;t linked to an active order,
            so it doesn&apos;t have portal access yet. If you believe this is a mistake, your
            advisor can help.
          </p>
