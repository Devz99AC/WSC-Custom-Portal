...
>
> Live at **`portal.wholesaleshelfcorporations.com`**, still behind Basic Auth, reading real
> Salesforce data. Six sections: **Orders · Payments · Documents · Learning Center · Support ·
> Profile**. Magic-link login delivered from `noreply@wholesaleshelfcorporations.com` over
> Resend's HTTPS API (Railway blocks outbound SMTP), with Reply-To `support@`. Typed Salesforce
> error classification. Gates green: typecheck 4/4, lint clean, build 3/3, 90 tests.
>
> **Step 9 (DNS) closed 2026-08-06 — the go-live blocker is gone.** What remains before real
> clients is step 14, removing Basic Auth, which is a CEO decision rather than a technical one.
> DNS is administered in **Cloudflare**; the full runbook and the rationale for choosing Resend
> over the company's existing SendGrid are in [`docs/dns-runbook.md`](docs/dns-runbook.md).
>
