...
>
> ⚠️ **Now that it is live on real data, four things are overdue** and were flagged to the stakeholder: (1) **Vercel Hobby→Pro** — the portal is commercial + public on the corporate domain and the free tier is non-commercial (suspension risk); (2) **the CEO checkpoint (step 14) never happened** — go-live occurred by action, not approval; (3) **Documents renders nothing in production** (Files vs `Attachment`, see below); (4) the **EIN least-privilege trim** — the integration user is now assigned a permset that still grants `EIN__c` read, which the BFF no longer selects. Also still open from step 12: the `vercel.json` preview-deploy defect.
>
