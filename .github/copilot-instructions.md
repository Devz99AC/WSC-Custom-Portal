...
>
> 🔒 **Security pass 2026-08-07** (code review, not a live pentest). Verified sound and worth not undoing: no secret reaches the browser (**zero `VITE_`/`import.meta.env` in the SPA**), session in an `httpOnly` cookie, `verifySessionJwt` pins `algorithms:["HS256"]` (blocks alg-confusion), magic-link tokens are 256-bit and stored **hashed** + single-use, row-level authz on all five endpoints reads identity from the cookie (**the client never sends an email or id to select data**), no account enumeration, Salesforce messages never reach the client, `Content-Disposition` strips CR/LF. Four things were added:
...
>
> Accepted by the stakeholder, deliberately not fixed: the portal being publicly reachable (see step 16) and the magic-link token appearing in Vercel/Railway access logs (single-use, 15min, logs are private).
>
