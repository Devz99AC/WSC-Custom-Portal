...
> 📌 **State as of 2026-08-07: 12/18.** Live at **`portal.wholesaleshelfcorporations.com`** (Vercel + Railway + Redis). Six sections: **Orders · Payments · Documents · Learning Center · Support · Profile**. Gates: typecheck 4/4, lint clean, build 3/3, 99 tests.
>
> **Fields removed from the order page 2026-08-07** at the stakeholder's request: **EIN issued date**, **credit score** and **funding capacity**. They came off the **DTO, the zod schema and the SOQL** too, not just the JSX — hiding them in the UI would have left them travelling in the JSON where any client could read them in devtools, and `Credit_Score__c`/`Funding_Capacity__c` are WSC's own assessment of the corp rather than a fact about it. Same commit: the "Product purchased" card shows **only the company name** (the `{age}-year aged {type}, Credit-Ready package.` blurb repeated two rows of the grid below it), and the **Incorporated** row reads `March 15, 2016 (8 Years Old)` — **the filing date stays**, with the age appended, because age is the product (§3, *Aged Corp*) and this saves the client doing the subtraction. Either half is dropped when Salesforce hasn't filled it in, so a corp with `Age__c = 0` shows the bare date rather than "0 Years Old".
>
