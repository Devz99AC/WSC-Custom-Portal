...
> 📌 **State as of 2026-08-06: 10/14.** Live at **`portal.wholesaleshelfcorporations.com`** (Vercel + Railway + Redis, still behind HTTP Basic Auth), reading real Salesforce data through a least-privilege integration user. Six sections: **Orders · Payments · Documents · Learning Center · Support · Profile**. Gates: typecheck 4/4, lint clean, build 3/3, 90 tests.
>
> **Step 9 (DNS) closed 2026-08-06 — the go-live blocker is gone.** Magic-link email now sends from `noreply@wholesaleshelfcorporations.com` via Resend, with Reply-To `support@`. DNS lives in **Cloudflare**. SendGrid was evaluated and rejected even though the domain is already authenticated there — its suppression lists are account-wide and WSC runs heavy marketing through it, so a client who bounced a campaign would silently never receive their sign-in link; the fix (a dedicated subuser) needs a paid plan. Full rationale in [`docs/dns-runbook.md`](docs/dns-runbook.md) §0.5.
>
...
>
> **What now stands between this and real clients is step 14 (removing Basic Auth), which needs the CEO's go-ahead** — not a technical blocker. Everything else open is either premature (4, 5), test debt (13), or parked on an open question.
>
