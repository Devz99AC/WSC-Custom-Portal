...
> 📌 **State as of 2026-08-06: 9/14.** Live in staging (Vercel + Railway + Redis, behind HTTP Basic Auth), reading real Salesforce data through a least-privilege integration user. Six sections: **Orders · Payments · Documents · Learning Center · Support · Profile**. Gates: typecheck 4/4, lint clean, build 3/3, 87 tests.
>
> **The total went from 15 to 14 on 2026-08-06:** Refer a Friend left the numbered sequence entirely and moved to the deferred write fence (W2). It had been split along the read/write seam — list now, form later — and the stakeholder called that off: *"let it to the Write phase even if it is a mock up, better to finish the current stuff."* Both halves come back together once Q3 defines the referral rules. **Do not rebuild the read half on its own.**
>
> **The only real blocker to going live is step 9 (DNS)** — it was step 10 until the 2026-08-06 renumbering. Until the sending domain is verified in Resend, magic-link emails reach nobody but the Resend account owner — so no real client can sign in, no matter what else is built. **Full step-by-step runbook: [`docs/dns-runbook.md`](docs/dns-runbook.md)** — DNS is administered in **Cloudflare** (nameservers `gina`/`mark.ns.cloudflare.com`), not in Vercel and not at the registrar; records added anywhere else do nothing. ⚠️ The domain already carries one SPF record covering Google, Salesforce and Formstack, and a domain may only have one — but Resend usually scopes its SPF to a `send.` subdomain and aligns via DKIM, so a merge is often unnecessary. The decision rule is in the runbook §1.2.
>
