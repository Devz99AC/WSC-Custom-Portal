...
> 📌 **State as of 2026-08-07: 11/18.** Live at **`portal.wholesaleshelfcorporations.com`** (Vercel + Railway + Redis). Six sections: **Orders · Payments · Documents · Learning Center · Support · Profile**. Gates: typecheck 4/4, lint clean, build 3/3, 94 tests.
>
> 🔓 **HTTP Basic Auth is GONE as of 2026-08-07** — `apps/web/middleware.ts` was deleted on the stakeholder's instruction, so the next deploy is open to anyone with the URL. Step 16 is **not** ticked, because its CEO checkpoint never happened and the portal still reads the **sandbox**: whoever signs in reaches sandbox data. Two loose ends left behind: the `BASIC_AUTH_USER`/`BASIC_AUTH_PASSWORD` vars in Vercel are now dead weight, and **the fix planned for the preview-deploy defect below is no longer available** — it was going to live in that middleware, and there is no middleware now. `@vercel/edge` is still in `apps/web/package.json`; dropping it needs a `pnpm install` to refresh the lockfile, or `--frozen-lockfile` deploys break.
>
> **Step 18 (mobile responsiveness + visual polish) closed 2026-08-06**, entirely inside `theme.css` and its existing tokens. Verified in headless Chrome at 375/768/1280 with a per-element horizontal-overflow audit across all eight views (0 overflows in 24 combinations). Three things there worth not undoing: the login input is **16px because anything smaller makes iOS Safari zoom the page** on focus; the five tables become labelled cards below 640px, which overrides `display` and therefore **requires the explicit ARIA roles now in the components**; and the mobile sidebar no longer hides `.side-foot`, which had left a phone user with **no way to sign out**. The logo is still set in type — there is no brand asset in the repo — but it now lives in one place, `apps/web/src/components/WscLogo.tsx`.
>
