...
> **Scope note:** This document specifies the target production system. Where a value is illustrative, it is drawn from the original prototype's sample order `#OO-1042` — real seeded orders are `UO1423102` / `UO1423103`.
>
> 📌 **What is actually built, and how far it has got, is [`execution-roadmap.html`](execution-roadmap.html) — 10/18 as of 2026-08-06.** This file describes the destination, not the current state; the line below claiming the repo "currently ships a static prototype" was true on 2026-07-22 and stopped being true long before that date. The repo now ships a pnpm monorepo (`apps/web` Vite+React, `apps/bff` Fastify, `packages/shared`) live on Vercel + Railway, with six portal sections reading real Salesforce data. The old prototype survives at `apps/web/public/prototype.html` as a design reference only.
>
> ⚠️ **RE-SCOPE 2026-07-22 ([ADR-0006](adr/0006-post-sale-scope-descope-payments.md)) — the stakeholder defined the FINAL scope: a post-sale service portal.** New purchases go through the human Sales rep; there is **no in-portal checkout**. Accordingly: Stripe payments are **DESCOPED and kept only as reference** (§1.1 row, §1.3 flow C, §4.2 Stripe row, §5.1 Stripe secrets — only payment *display* remains), and any client-upload document path is descoped (Documents is view/download + sign; storage TBD pending discovery of where documents live in the org — likely Salesforce Files). **E-signature STAYS** with a provider swap: **Formstack Documents** replaces DocuSign/PandaDoc (already-paid-tools rule; conditional on the Formstack plan covering signing — no Formstack package is installed in the org, so the BFF talks to Formstack's own API). Flow D / §4.2 / §5.1 apply with Formstack endpoints via the same `IESignProvider` port. Added to scope: multi-order structure, Support (tickets/WhatsApp/chat — org already has LiveChat + Twilio packages installed), Refer-a-Friend, Learning Center. (The P1–P6 phase list in `ACTION-PLAN.md` was superseded on 2026-07-24 by the read-vs-write sequencing in [`execution-roadmap.html`](execution-roadmap.html); Support tickets in particular moved to *deferred* once `Case` turned out to be unused in this org's production — open question Q5.)
